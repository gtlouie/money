...
class hyNeedData:
   ...
   #机器人循环模拟用户操作
    def ai_mock_clientOpt(self):
        #如果程序启动，机器每隔多少分钟模拟用户操作，
        sign = 0;
        while sign < 1:
            #如果时间在9：00 到11：30分，每隔几分钟，模拟用户操作，将数据写入excel中，如果11：30~12：59：59，无需写入数据，如果时间在1：00~3：00 ，重新开始写入数据，3：00后无需写入数据
            amTimeS = datetime.datetime.strptime(str(datetime.datetime.now().date()) + '9:00', '%Y-%m-%d%H:%M')
            amTimeE = datetime.datetime.strptime(str(datetime.datetime.now().date()) + '11:30', '%Y-%m-%d%H:%M')
            pmTimeS = datetime.datetime.strptime(str(datetime.datetime.now().date()) + '13:00', '%Y-%m-%d%H:%M')
            pmTimeE = datetime.datetime.strptime(str(datetime.datetime.now().date()) + '15:00', '%Y-%m-%d%H:%M')

            # 获取当前请求时间
            nowTime = datetime.datetime.now()
            if nowTime<amTimeS:
                print("股市还没开市，请稍等。。。。。。。")
            elif nowTime>=amTimeS and nowTime<=amTimeE:
                #模拟用户操作
                print("上午股市已开始。。。。。")
                hyNeedData().mock_clientOpt()
                time.sleep(sleepTime)

            elif nowTime>amTimeE and nowTime<pmTimeS:
                print("午休时间，闭市中！！！！！")

            elif nowTime>=pmTimeS and nowTime<=pmTimeE:
                 print("下午股市已开始。。。。")
                 hyNeedData().mock_clientOpt()
                 time.sleep(sleepTime)

            elif nowTime>pmTimeE:
                print("今天股市已收盘！！！！！")
                sign=1
